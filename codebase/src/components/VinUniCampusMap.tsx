...
export default function VinUniCampusMap({
  ...
  const [selectedId, setSelectedId] = useState(() => CAMPUS_LOCATIONS.some((item) => item.id === initialDestinationId) ? initialDestinationId : '');
  const [query, setQuery] = useState('');
  const [zoom, setZoom] = useState(1);

  useEffect(() => {
    setSelectedId(CAMPUS_LOCATIONS.some((item) => item.id === initialDestinationId) ? initialDestinationId : '');
  }, [initialDestinationId]);
  ...
  const visibleLocations = useMemo(() => {
    ...
    return CAMPUS_LOCATIONS.filter((item) =>
      normalizeSearch(`${item.name} ${item.subtitle ?? ''} ${item.label}`).includes(normalized),
    );
  }, [query]);
    ...
    <section className="campus-map" aria-label="Bản đồ VinUni">
      <div className="campus-map__toolbar">
        ...
        <div className="campus-map__zoom" aria-label="Điều khiển thu phóng">
          <button type="button" aria-label="Phóng to bản đồ" disabled={zoom >= 2} onClick={() => setZoom((value) => Math.min(2, value + 0.25))}>+</button>
          <button type="button" aria-label="Thu nhỏ bản đồ" disabled={zoom <= 1} onClick={() => setZoom((value) => Math.max(1, value - 0.25))}>−</button>
        </div>
      </div>
      ...
      <div className="campus-map__viewport">
        <div className="campus-map__canvas" style={{ '--map-zoom': zoom } as CSSProperties}>
            ...
            {CAMPUS_LOCATIONS.map((location) => {
              ...
              return (
                <g
                  ...
                  onKeyDown={(event) => {
                    if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); selectLocation(location); }
                  }}
                >
...
function normalizeSearch(value: string): string {
  return value.toLowerCase().replace(/đ/g, 'd').normalize('NFD').replace(/\p{M}/gu, '');
}
